...
from app.schemas.credit_report_read import CofaceReportReadCreate
...
def _resolve_validation(
    ...
    if not report_document or customer_document != report_document:
        return (
            "invalid",
            False,
    ...
    if warnings:
        return (
            "valid_with_warnings",
            True,
            "Relatório COFACE validado com alertas de leitura.",
        )

    return ("valid", True, "Relatório COFACE validado e pronto para análise.")
...
def create_coface_report_read(db: Session, payload: CofaceReportReadCreate) -> CreditReportRead:
    ...
    entry = CreditReportRead(
        ...
        validation_message="Processando leitura do relatório COFACE.",
        score_primary=None,
        score_source=None,
        warnings_json=[],
        confidence=None,
    ...
    try:
        ...
        read_result = read_coface_report(raw_text)
        ...
        entry.status = "error"
        entry.validation_message = "Não foi possível ler o relatório COFACE."
        entry.is_document_match = False
        entry.warnings_json = ["Falha ao extrair e ler o relatório COFACE."]
