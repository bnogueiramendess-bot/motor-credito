...
export async function POST(request: Request) {
  ...
  if (!response.ok) {
    return NextResponse.json({ detail: data.detail ?? "Falha no login." }, { status: response.status });
  }
  ...
  store.set("gcc_user_role", String(data?.user?.role ?? ""), { httpOnly: false, sameSite: "lax", path: "/" });
  store.set("gcc_is_administrator", String(Boolean(data?.user?.is_administrator)), { httpOnly: false, sameSite: "lax", path: "/" });
  store.set("gcc_can_import_ar_aging", String(Boolean(data?.user?.can_import_ar_aging)), { httpOnly: false, sameSite: "lax", path: "/" });
