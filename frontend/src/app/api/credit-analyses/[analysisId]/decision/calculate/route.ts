...
export async function POST(_: Request, context: Context) {
  ...
  try {
    const payload = await fetchBackend(`/credit-analyses/${analysisId}/decision/calculate`, {
      method: "POST",
      body: JSON.stringify({}),
    });
    ...
    return NextResponse.json({ detail: "Falha ao calcular decisão técnica." }, { status: 500 });
  }
}
