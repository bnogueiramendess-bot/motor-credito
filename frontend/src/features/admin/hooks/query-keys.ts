export const adminQueryKeys = {
  users: ["admin", "users"] as const,
  workflowRoles: ["admin", "workflow-roles"] as const,
  approvalMatrix: ["admin", "approval-matrix"] as const,
  approvalMatrixNextCode: ["admin", "approval-matrix-next-code"] as const,
  approvalMatrixOptions: ["admin", "approval-matrix-options"] as const,
  committees: ["admin", "committees"] as const,
  committeeOptions: ["admin", "committee-options"] as const,
  committeeNextCode: ["admin", "committee-next-code"] as const,
  businessUnits: ["admin", "business-units"] as const,
  roleMatrix: ["admin", "role-matrix"] as const,
  profiles: ["admin", "profiles"] as const
};
