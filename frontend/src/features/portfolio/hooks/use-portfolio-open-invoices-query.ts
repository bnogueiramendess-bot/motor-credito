...
export function usePortfolioOpenInvoicesQuery(params: UsePortfolioOpenInvoicesQueryParams) {
  return useQuery({
    queryKey: ["portfolio-open-invoices", params.economicGroup ?? null, params.cnpj ?? null],
    queryFn: async () => {
      if (params.economicGroup) {
        return getPortfolioGroupOpenInvoices(params.economicGroup);
      }
      if (params.cnpj) {
        return getPortfolioCustomerOpenInvoices(params.cnpj);
      }
      return [];
    },
