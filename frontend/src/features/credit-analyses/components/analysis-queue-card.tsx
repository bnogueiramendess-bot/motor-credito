    ...
    <article className="rounded-[14px] border border-[#D7E1EC] bg-white p-4">
      <div className="mb-3 flex items-start justify-between gap-3">
        <div>
          ...
          <p className="text-[11px] text-[#4F647A]">{item.cnpj ?? "CNPJ nÃ£o informado"} â€¢ {item.analysis_code}</p>
          <p className="text-[11px] text-[#8FA3B4]">Grupo: {item.economic_group ?? "-"} â€¢ BU: {item.business_unit ?? "-"}</p>
        </div>
      ...
      <div className="grid gap-3 md:grid-cols-3">
        <div className="rounded-[10px] bg-[#F8FAFC] p-3 text-[11px]">
          <p className="text-[#8FA3B4]">SolicitaÃ§Ã£o</p>
        ...
        <div className="rounded-[10px] bg-[#F8FAFC] p-3 text-[11px]">
          ...
          <p className="text-[#102033]">Limite disponÃ­vel: {formatCurrency(item.available_limit ?? 0)}</p>
          <p className="text-[#102033]">Valor em aberto: {formatCurrency(item.open_amount ?? 0)}</p>
        </div>
        <div className="rounded-[10px] bg-[#F8FAFC] p-3 text-[11px]">
          <p className="text-[#8FA3B4]">GovernanÃ§a e relatÃ³rios</p>
          ...
          {item.previous_analysis_id ? <p className="text-[#4F647A]">AnÃ¡lise anterior: #{item.previous_analysis_id}</p> : null}
        </div>
      </div>

      <div className="mt-3 flex flex-wrap items-center gap-2">
        <Link href={`/analises/${item.analysis_id}`} className="rounded-[8px] bg-[#1E3A8A] px-3 py-1.5 text-[11px] font-medium text-white">Abrir anÃ¡lise</Link>
        {canExecuteAnalysis ? <Link href={`/analises/${item.analysis_id}`} className="rounded-[8px] border border-[#D7E1EC] px-3 py-1.5 text-[11px] font-medium text-[#4F647A]">Continuar anÃ¡lise</Link> : null}
        <Link href={`/analises/${item.analysis_id}`} className="rounded-[8px] border border-[#D7E1EC] px-3 py-1.5 text-[11px] font-medium text-[#4F647A]">Importar relatÃ³rios</Link>
        {canViewDossier ? <Link href={`/credit-analysis/${item.analysis_id}/dossier`} className="rounded-[8px] border border-[#D7E1EC] px-3 py-1.5 text-[11px] font-medium text-[#4F647A]">Gerar dossiÃª</Link> : null}
        {canSubmitRequest ? <Link href={`/analises/${item.analysis_id}`} className="rounded-[8px] border border-[#D7E1EC] px-3 py-1.5 text-[11px] font-medium text-[#4F647A]">Submeter para aprovaÃ§Ã£o</Link> : null}
      </div>
    </article>
  );
}
