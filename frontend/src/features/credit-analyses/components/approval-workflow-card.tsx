...
export function ApprovalWorkflowCard({ analysisId, summary, availableActions, mode = "decision" }: ApprovalWorkflowCardProps) {
  ...
  const actionMutation = useMutation({
    mutationFn: (payload: { action: DossierDecisionAction; justification?: string | null }) =>
      payload.action === "escalate_to_committee"
  ...
  return (
    <section className="rounded-[18px] border border-[#D7E1EC] bg-white p-5 shadow-[0_10px_24px_rgba(10,29,64,.06)]">
          ...
          <div className="space-y-2">
            {progress.length ? progress.map((step, index) => (
              <div key={`${step.role_code ?? step.role_label}-${index}`} className="flex gap-3 rounded-[12px] border border-[#E6ECF2] bg-[#FBFCFE] p-3">
                <span className={`mt-0.5 inline-flex h-7 min-w-7 items-center justify-center rounded-full border text-[12px] font-bold ${approvalStatusClass(step.status)}`}>{step.status.toLowerCase() === "approved" ? "✓" : (step.status.toLowerCase() === "active" || step.status.toLowerCase() === "in_committee") ? "⏳" : "○"}</span>
      ...
      {canDecide ? (
        <div className="mt-5 flex flex-wrap items-center justify-between gap-3 border-t border-[#E6ECF2] pt-4">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            {canEscalate ? <button type="button" onClick={() => setModalAction("escalate_to_committee")} disabled={actionMutation.isPending} className="rounded-lg bg-[#92400E] px-4 py-2 text-[12px] font-semibold text-white disabled:opacity-50">Submeter ao Comite</button> : null}
      ...
      {modalAction ? <div className="fixed inset-0 z-40 flex items-center justify-center bg-[#0D1B2A]/55 p-4" onClick={() => setModalAction(null)}><div className="w-full max-w-[560px] rounded-[14px] border border-[#D7E1EC] bg-white p-5 shadow-xl" onClick={(event) => event.stopPropagation()}><h3 className="text-[20px] font-semibold text-[#102033]">{modalAction === "reject" ? "Rejeitar analise" : modalAction === "escalate_to_committee" ? "Submeter ao Comite" : "Solicitar ajustes"}</h3><textarea value={comment} onChange={(event) => setComment(event.target.value)} rows={5} className="mt-4 w-full rounded-[10px] border border-[#D7E1EC] px-3 py-2 text-[12px] text-[#102033] outline-none focus:border-[#94A3B8]" placeholder={modalAction === "escalate_to_committee" ? "Explique por que esta operacao deve ser analisada pelo Comite de Credito." : "Descreva a justificativa..."} /><div className="mt-4 flex justify-end gap-2"><button type="button" onClick={() => setModalAction(null)} className="h-9 rounded-[10px] border border-[#D7E1EC] bg-white px-3 text-[12px] font-medium text-[#475569]">Cancelar</button><button type="button" onClick={submitModalAction} disabled={actionMutation.isPending} className="h-9 rounded-[10px] bg-[#334155] px-3 text-[12px] font-medium text-white disabled:opacity-50">{modalAction === "escalate_to_committee" ? "Submeter ao Comite" : "Confirmar"}</button></div></div></div> : null}
    </section>
  );
}
